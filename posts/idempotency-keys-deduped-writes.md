...
Number three is the one people skip.
...
In Postgres specifically, you want `INSERT ... ON CONFLICT (key) DO NOTHING RETURNING *`. If the returning set is empty, you lost the race; fetch the existing row. If it has a row, you won; you own the key.
...
- **Expire after some window.** 24 hours is a reasonable default for most APIs, and [Stripe uses 24 hours](https://docs.stripe.com/api/idempotent_requests). Long enough to cover any reasonable retry window, short enough that the table doesn't grow without bound.
...
This pattern handles **duplicate writes to one service**. It does not handle:
...
- **Replay attacks.** If an attacker observes a request, they can re-use the key, but that's the attacker forging a request. Idempotency assumes the key is a *correctness* tool, not a *security* tool.
...
1. **Idempotency is about deduping side effects, not replaying responses.** The "return the cached response" part is convenient. The "don't double-execute" part is the point.
...
I keep coming back to this pattern because the moment you have a write endpoint that can be called from a flaky network (which is every write endpoint, ever), you need it.
