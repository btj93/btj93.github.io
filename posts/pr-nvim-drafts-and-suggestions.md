...
What shipped in October:

- Persistent drafts.
- Suggestion blocks, rendered as readable diffs and applyable in place.
...
What if you opened the same thread in two Neovim instances, and edited the draft in both?
...
It's cruder than a CRDT, but in practice the user opens both instances, sees the warning, picks one, copy-pastes the other in if they want, and moves on.
...
1. **Render the suggestion as a diff** rather than a raw fenced code block, when the popup displays the comment.
...
This took one cycle of getting it wrong before I landed on it. My first version auto-committed the suggestion. People (correctly) hated this.

### Authoring suggestions from visual mode

The other half is *creating* a suggestion. It's a small affordance that has paid off:
...
You edit the lines, save, and pr.nvim posts it as a review comment on those lines. The thing that took the longest here was figuring out what "those lines" map to in the *PR's* diff coordinates, because the buffer line numbers are local to the working tree.
...
Fire a `User` event, let consumers re-render. That keeps the winbar decoupled, and statusline plugins can hook the same event without any pr.nvim-specific glue.
...
The plugin is now usable enough that I'm starting to use it for non-trivial PRs at work. The next stretch of work is:
