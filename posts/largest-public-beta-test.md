...
A friend hit the thumbs-up button on a chatbot response over coffee last week. They did it without thinking — the answer was good, the button was right there, the gesture was as light as liking a post on a social feed.
...
LLM "betas" — and I'm going to call them that for the rest of this post — are different in two ways. They're vastly larger in scale, and the artifact of the iteration is fundamentally different in kind.

## How an LLM actually gets better

A simplified mental model, for context:
...
Aggregated across the user base, this is an absurd amount of behavioral information. Think about it from the provider's perspective: traditional product feedback — surveys, NPS scores, user interviews — is slow, expensive, and badly biased toward who actually fills out the survey. What an LLM provider gets instead is a billion-person, 24/7 UX research panel, with real tasks, real frustration, and real moments of "yes, that one".

Some of that flows into the next round of preference learning. Some of it flows into the evaluation sets that determine which model gets shipped. Some of it just informs product decisions ("turns out half our usage is X, who knew"). All of it is more granular and more honest than anything the product-research field has ever had access to.

## The genuinely novel thing

Here's the part I keep finding interesting.
...
It's not exactly authorship — the influence is statistical, anonymized, and indirect. It's not exactly tagging or labeling either — most of the signal is behavioral, not deliberate. It's something else, and I don't think we have a clean word for it yet. "Participant" works. "Beta tester" works. "Co-author by accumulation" might be closer.

## What you give and what you get

I think it's worth being honest about both sides.

**What you get:** a tool that, by any reasonable measure, is improving faster than any previous category of software. The Claude or ChatGPT you used a year ago is dramatically worse than what you have now, and the change is concentrated in the post-training stages — which is to say, the parts shaped by the feedback loop. The capabilities you depend on this year for free wouldn't exist in their current form without a year of usage data.
...
A practical thing to know, because the defaults vary wildly:
...
If you've never looked at your provider's data settings, look at them. Not because the answer is necessarily to opt out — for a lot of casual usage, the bargain is genuinely fine — but because you should know what you're agreeing to before you click thumbs-up.

## Some lessons I keep coming back to

A few takeaways from spending a year watching this play out as both a user and an engineer:

1. **The norms aren't settled yet.** What's acceptable practice in 2026 is going to look quaint, in one direction or the other, by 2030. Pay attention.
...
5. **Your thumbs-up is a vote.** Tiny, statistical, anonymized — but multiplied by however many people share your taste, it actually changes what the next model says to someone else. That's a strange amount of authorship to give away without thinking about it.

I'm not arguing for or against participating. The fact is that I am participating. So is almost everyone reading this. The thing I want is for us to know we're doing it — and to take a beat, occasionally, before we click the button.

We are early. This is the part of the era that will look weird in hindsight. The norms will arrive late. The defaults will shift. The opt-outs will become opt-ins or the other way around.

Until then, the most useful thing I can tell you is: read the data settings. Make a choice. Don't sleepwalk through it.

And, when the model nails something — when it actually understood what you meant on the first try, when it caught a bug you missed, when it wrote a paragraph better than you'd have written it — give it the thumbs-up. You're not just rating a response. You're voting on what the model will be like for the next person.
